...
# the enchanting table. In this case we're checking for a custom curse and for that reason we
...
# curse this new item can get at "complement:register", in this case we use "Build.ForItem.item"
...
execute if entity @s[tag=tepcom.custom_id_tag] if data storage teplus:loot Build.ForItem.item run function enchantplus:loot/enchanting/set_curse/success